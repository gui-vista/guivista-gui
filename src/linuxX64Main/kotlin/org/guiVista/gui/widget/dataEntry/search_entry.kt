...
public actual class SearchEntry(searchEntryPtr: CPointer<GtkSearchEntry>? = null) : EntryBase {
    ...
    /**
     * Connects the *next-match* event to a [handler] on a [SearchEntry]. This event is a key binding event that is
     * used when the user initiates a move to the next match for the current search String. Applications should connect
     * to it to implement moving between matches. The default bindings for this event is *Ctrl-g*.
     * @param handler The event handler for the event.
     * @param userData User data to pass through to the [handler].
     */
    public fun connectNextMatchEvent(handler: CPointer<NextMatchHandler>, userData: gpointer): ULong =
        connectGSignal(obj = gtkSearchEntryPtr, signal = SearchEntryEvent.nextMatch, slot = handler, data = userData)
    ...
    public fun connectStopSearchEvent(handler: CPointer<StopSearchHandler>, userData: gpointer): ULong =
        connectGSignal(obj = gtkSearchEntryPtr, signal = SearchEntryEvent.stopSearch, slot = handler, data = userData)
