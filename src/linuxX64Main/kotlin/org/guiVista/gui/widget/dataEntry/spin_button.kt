...
public actual class SpinButton(spinButtonPtr: CPointer<GtkSpinButton>? = null, climbRate: Double = 0.0,
    ...
    public fun connectValueChangedEvent(handler: CPointer<ValueChangedHandler>, userData: gpointer): ULong =
        connectGSignal(obj = gtkSpinButtonPtr, signal = SpinButtonEvent.valueChanged, slot = handler, data = userData)
     ...
     * need to control the cursor programmatically. The default bindings for this event are *Up*, *Down*, *Page Up*, and
     * *Page Down*.
     * @param handler The event handler for the event.
     * @param userData User data to pass through to the [handler].
     */
    public fun connectChangeValueEvent(handler: CPointer<ChangeValueHandler>, userData: gpointer): ULong =
        connectGSignal(obj = gtkSpinButtonPtr, signal = SpinButtonEvent.changeValue, slot = handler, data = userData)
...
public typealias ChangeValueHandler = CFunction<(
    spinButton: CPointer<GtkSpinButton>,
